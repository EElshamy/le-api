...
import { MoneyScalar } from './money.scaler';
...
export function generateGqlResponseType<T, K>(
  ...
  type DataType =
    ...
    : K extends boolean ? DataTypeAsArray
    : T extends any[] ? DataPaginatedType
    : DataSingleItemType;

  const filedType =
    isRawArray ? TClass
    : Array.isArray(TClass) ? generateGqlPaginationResponseType<T>(TClass)
    : TClass;
  const className =
    isRawArray ? `${TClass[0].name}sArray`
    : Array.isArray(TClass) ? `${TClass[0].name}s`
    : TClass.name;
...
export const GqlMoneyResponse = generateGqlResponseType(MoneyScalar);
